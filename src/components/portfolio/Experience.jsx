...
const ExperienceCard = ({ experience, index }) => {
  ...
  const dateRange = `${formatDate(experience.start_date)} - ${experience.is_current ? "Present" : formatDate(experience.end_date)}`;
  ...
  return (
    <VerticalTimelineElement
      ...
      date={dateRange}
      dateClassName="text-white/60 font-medium text-xs md:mx-4"
      iconStyle={{
        background: experience.icon_bg || "linear-gradient(135deg, #667eea 0%, #764ba2 100%)",
      ...
      icon={
        <div className="flex justify-center items-center w-full h-full">
          {experience.company_logo ? (
            ...
            <FiBriefcase className="text-white w-1/2 h-1/2" />
          )}
        </div>
      }
    >
      ...
      >
        ...
        <ul className="space-y-2">
          {(experience.responsibilities || []).map((point, pointIndex) => (
            <motion.li
...
const Experience = () => {
  const { experiences: supabaseExperiences, loading } = usePortfolioData();

  const experienceData = supabaseExperiences && supabaseExperiences.length > 0
    ? supabaseExperiences
    : defaultExperiences;

  if (loading) {
    return <div className="text-white text-center py-20">Loading career data...</div>
  }
