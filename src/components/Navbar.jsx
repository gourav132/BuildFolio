...
const Navbar = ({ isExpanded, handleBackPreview }) => {
  const contextValue = useContext(PreviewContext);
  // Safely handle context being undefined or null (though strictly it should exist if wrapped)
  const previewData = contextValue ? contextValue[0] : {};
  ...
  // Use displayName from context or fallback to "Portfolio"
  const name = previewData?.displayName || "Portfolio";
